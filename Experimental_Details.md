...
<br>
Baseline models’ hyperparameters were also tuned, in regards to embedding size, learning rate, and dropout ratio. Specific details like the optimal hyperparameter values, hyperparameter search range are specified in [hyper](Baseline_models(Others)/hyper) and [hyper_result](Baseline_models(Others)/hyper_result)folder.
