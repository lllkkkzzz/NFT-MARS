    ...
    def __init__(self, args):
        ...
        if self.model_name == 'MMMO':
            ...
            self.model = MMMO(self.features, self.user_feat, self.edge_index, self.batch_size, self.num_user, self.num_item,
                               self.reg_parm, self.dim_x, self.attention_dropout, self.data_path).cuda()
    ...
    def run(self):
        ...
        for epoch in range(self.num_epoch):
            ...
            with torch.no_grad():
                ...
                recall, ndcg = recall[1], ndcg[1] # top10 only
                # Test
                recall_test, ndcg_test = self.model.accuracy(self.test_dataset, self.indices_test)
                recall_test, ndcg_test = recall_test[1], ndcg_test[1] # top10 only
            ...
            if recall > best_recall: 
                ...
                #     with open(self.PATH_weight_save + self.model_name + self.collection + str(self.number) + '_attention_score' + '.pickle', 'wb') as f:
            ...
            epoch = {'loss':loss_avg, 'loss_BPR': loss_BPR_avg, 'loss_Price': loss_Price_avg, 
                     ...
                     'recall_test': recall_test, 'ndcg_test': ndcg_test}
            # wandb.log(epoch)
...
if __name__ == '__main__':
    ...
    mmmo = Net(args)
    # wandb.init(project="2023-03-20", name=f'{args.collection}_{args.SWA}', config=state)
    mmmo.run()
    # wandb.finish()